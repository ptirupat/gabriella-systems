...
| this PR (2026-09-16) | HTML live samples + conflict-marker fix. Hero *set* unchanged. GIF binaries not regenerated. |

**2026-09-16 HTML lock (live samples; hero set unchanged):** homepage `.hero-metric-card` tiles are **Bat speed at impact 16 km/h** + **Contact time in this clip 817 ms** + **Run-up speed 20.4 km/h** only. Ball **122 km/h**, Arm angular speed **619 °/s**, Head stability **41.4 cm**, and Front knee **155°** are clip-HUD / session-progress **detail** only — not homepage heroes. **Release height is omitted** until a calibrated gated overarm value exists — do not show Can't measure / can't be determined / Measured when calibration supports it; do not publish 1.09 m or invent 2.1 m. Do not republish ~22 / 400 ms / 21.6 / 22.4 / ~1840°/s as public samples. Contract: Modal PRODUCT.md. GIF binaries were not regenerated in this pass.
...
**2026-09-16 HTML lock (batting impact speed; superseded for sample numbers):** batting homepage *heroes* remain **Bat speed at impact** (`bat_speed_at_impact_kmh`) + **Contact time in this clip** (`impact_offset_ms`). That pass still published ~22 / 400 ms; public samples are now **16 km/h** + **817 ms**. Label is **Bat speed at impact** — never bare “bat speed,” never “pipeline peak.” Do **not** publish peak ~90 or the old pipeline-peak **36–39 km/h** sample. Head stability is batting detail/capability copy only (live **41.4 cm**). Front stride is not shown on homepage or batting progress/hero rows.
...
2. **KAN-272** (bowling fail-loud / run-up) remains open. Do not invent bowling numbers. Public bowling *hero* is Run-up only.
...
- Conflicting bat-speed units (homepage km/h vs batting.html mph) or unlabelled mph cards. Use one Showcase-like **km/h** **Bat speed at impact** sample (**16**). Never pipeline-peak 36–39 or peak ~90. Do not republish ~22 / 400 ms / 21.6 / 22.4 / ~1840°/s as public samples.
