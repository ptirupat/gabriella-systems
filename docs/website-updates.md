...
**2026-09-17 HTML lock (live-tip samples; hero set unchanged):** homepage `.hero-metric-card` tiles are **Bat speed at impact 10.6 km/h** + **Head stability 45.9 cm** + **Run-up speed 20.4 km/h** only. Contact **800 ms**, Ball **122 km/h**, Arm angular speed **619 °/s**, and Front knee **155°** are clip-HUD / session-progress **detail** only — not homepage heroes. Fail loud: ungated Head → **Can't measure** on that hero slot; do **not** put Contact back on the hero row. Tip strings unchanged. Head Δ −11.1 (pre 27.5 / post 16.4) is not shown. Contract: Modal PRODUCT.md. GIF binaries were not regenerated in this pass.
