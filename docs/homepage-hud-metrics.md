...
5. Homepage *heroes* have no ball speed, no Δ ball speed, no arm speed, no contact time, and no pipeline-peak bat speed. Bowling homepage hero is Run-up only and omits `ball_speed_kmh`. Homepage run-up is `peak_runup_speed_kmh` only.
